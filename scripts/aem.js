...
function decorateLinkedPictures(block) {
  // Handle existing picture + br + a pattern
  ...
  // Handle div with two p children: first contains picture, second contains link
  const targetDivs = block.querySelectorAll('div.default-content-wrapper');

  targetDivs.forEach((wrapper) => {
    const { children } = wrapper;

    // Check if wrapper has exactly 2 children, both p elements
    if (children.length === 2
      ...
      // Check if first p contains picture and second p contains link
      if (picture && link) {
        ...
        // Move picture into the new link
        firstP.insertBefore(newLink, picture);
        newLink.appendChild(picture);

        // Remove the second p element (the one with the link)
        secondP.remove();
      }
    }
  });
...
/**
 * MOVIDO PARA SCRIPTS.JS para tratamento de metadados mais específico.
 * MOVED TO SCRIPT.JS for more specific metadata handling.
...
async function loadHeader(header) {
  ...
  // Wait for header block to be fully loaded before applying logo linking
  await loadBlock(headerBlock);
  decorateLinkedPictures(headerBlock);
  return headerBlock;
}
